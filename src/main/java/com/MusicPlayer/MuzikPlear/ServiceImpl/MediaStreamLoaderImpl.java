package com.MusicPlayer.MuzikPlear.ServiceImpl;
...
public class MediaStreamLoaderImpl implements MediaStreamLoader {
    ...
    public ResponseEntity<StreamingResponseBody> loadEntireMediaFile(String localMediaFilePath) throws IOException {
        ...
        if(!filePath.toFile().exists()){
            throw new FileNotFoundException("The media file doesn't exists.");
        }
    ...
    public ResponseEntity<StreamingResponseBody> loadPartialMediaFile(String localMediaFilePath, String rangeValues) throws IOException{
        if(!StringUtils.hasText(rangeValues)){
            ...
            long fileSize = Files.size(filePath);
            System.out.println("Read range seeking values.");
            ...
            if(dashPos>0 && dashPos<=(rangeValues.length()-1)){
                ...
                if(rangesArr != null && rangesArr.length > 0){
                    System.out.println("Array size: "+rangesArr.length);
                    if(StringUtils.hasText(rangesArr[0])){
                        System.out.println("Range Values[0]: ["+rangesArr[0]+"]");
                    ...
                    if(rangesArr.length > 1){
                        System.out.println("Range Value[1]: ["+ rangesArr[1] + "]");
